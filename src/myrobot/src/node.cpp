...
//volatile double jointCommand[3];        // The goal position
...
*/
void jointCallback(sensor_msgs::JointState msg)
{
  jointCommand = msg;
...
int main(int argc, char **argv)
{
  ...
  //Construct a group using 5 known modules
  AddGroupFromNamesSrv add_group_srv;
  add_group_srv.request.group_name = group_name;
  add_group_srv.request.names = {"motor1", "motor2", "motor3", "motor4", "motor5"};
  ...
  ros::Subscriber joint_command_Subscriber = n.subscribe("/ikin_joint_states", 100, jointCallback);
  ...
  while(ros::ok())
    {
      ...
      // command_msg.position[0] = jointCommand.position;
      // command_msg.velocity[0] = jointCommand.velocity;
      // command_msg.effort[0]   = jointCommand.effort;
      command_publisher.publish(jointCommand);
